...
from app.services.schema_management_service import (
    get_schema_for_user,
    get_all_schemas_for_user,
...
@router.post("/file", response_model=List[IngestionResponse])
...
@router.get(
    "",
    response_model=List[SchemaViewResponse],
)
async def view_schema(
    current_user=Depends(get_current_user),
):
    mappings = await get_all_schemas_for_user(
